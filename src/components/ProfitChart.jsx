...
export default function ProfitChart({
  profitChartData, chartPeriod, setChartPeriod,
  stats = {}, theme, onClose, items = [],
}) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        {/* Header */}
        <div className={`flex items-center justify-between px-6 py-4 border-b ${theme.panelBorder} shrink-0`}>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className={`ml-auto text-sm font-mono font-semibold ${totalInPeriod >= 0 ? 'text-profit' : 'text-loss'}`}>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={cumulativeData}>
                  ...
                  <Tooltip
                    ...
                    formatter={v => [`$${parseFloat(v).toFixed(2)}`, 'Cumulative']}
                  />
                  <Line
                    type="monotone"
                    dataKey="cumulative"
          ...
          <div className="grid grid-cols-3 gap-3">
            {summaryCards.map(card => (
              <div key={card.label} className={`${theme.card} border ${theme.cardBorder} rounded-xl p-4 text-center`}>
                <p className="text-xs text-slate-600 mb-1.5">{card.label}</p>
                <p className={`text-base font-mono font-semibold ${
                  card.colored
                    ? (card.val ?? 0) >= 0 ? 'text-profit' : 'text-loss'
                    : 'text-slate-200'
                }`}>
                  {card.value}
                </p>
          ...
          {/* Daily P&L heatmap */}
          <div>
            <p className="text-xs text-slate-600 mb-3 uppercase tracking-wide">Daily P&L — Last 90 days</p>
            <div className="flex flex-wrap gap-1">
              {heatmapDays.map(d => {
                const intensity = d.profit === 0 ? 0 : Math.min(Math.abs(d.profit) / maxAbs, 1);
                return (
                  <div
                    key={d.date}
