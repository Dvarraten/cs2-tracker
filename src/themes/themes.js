// SAP Fiori Morning Horizon chart palette (SAP/theming-base-content)
export const SAP_CHART_COLORS = {
  1:  "#168eff", // blue
  2:  "#c87b00", // orange
  3:  "#75980b", // olive green
  4:  "#df1278", // magenta
  5:  "#8b47d7", // purple
  6:  "#049f9a", // teal
  7:  "#0070f2", // royal blue
  8:  "#cc00dc", // violet
  9:  "#798c77", // muted sage
  10: "#da6c6c", // salmon
  11: "#5d36ff", // indigo
  12: "#a68a5b", // warm tan
  good:     "#30914c",
  bad:      "#f53232",
  critical: "#e26300",
  neutral:  "#758ca4",
};

export const themes = {
  // ── 1. Midnight Blue (default) ──────────────────────────────────────────
  default: {
    name: "Midnight Blue",
    dot: "bg-blue-500",
    dotColor: "#3b82f6",
    bg: "bg-[#0c1120]",
    card: "bg-[#151f35]",
    cardHover: "hover:border-slate-500/50",
    cardBorder: "border-slate-600/40",
    panel: "bg-[#111827]",
    panelBorder: "border-slate-600/40",
    input: "bg-[#1a2540] border-slate-600/50",
    inputSell: "bg-[#1a2540] border-slate-600/50",
    tabActive: "bg-blue-600 text-white",
    tabInactive: "bg-[#151f35] text-slate-300 hover:bg-slate-700/50 border border-slate-600/40",
    accent: "text-blue-400",
    accentBg: "bg-blue-600 hover:bg-blue-500",
    subtext: "text-slate-400",
    text: "text-white",
    textSecondary: "text-slate-300",
    textHover: "hover:text-white",
    itemHoverBg: "hover:bg-white/8",
    soldCard: "bg-[#1a2540] border-slate-600/30",
    sidebarItem: "bg-slate-700/50 hover:bg-slate-600/50 border-slate-600/50",
    header: "bg-gradient-to-b from-[#1e2d4a] to-[#0f1929] border border-slate-700/40",
    chartGrid: "#334155",
    chartAxis: SAP_CHART_COLORS.neutral,
    chartTooltipBg: "#1e293b",
    chartTooltipBorder: "#475569",
    chartLine: SAP_CHART_COLORS[1],
    chartColors: [SAP_CHART_COLORS[1], SAP_CHART_COLORS[6], SAP_CHART_COLORS[3], SAP_CHART_COLORS[5], SAP_CHART_COLORS[2], SAP_CHART_COLORS[4]],
  },

  // ── 2. Dark ─────────────────────────────────────────────────────────────
  dark: {
    name: "Dark",
    dot: "bg-[#3b82f6]",
    dotColor: "#111318",
    bg: "bg-[#0a0b0f]",
    card: "bg-[#111318]",
    cardHover: "hover:border-[#1e2028]/80",
    cardBorder: "border-[#1e2028]",
    panel: "bg-[#0d1117]",
    panelBorder: "border-[#1e2028]",
    input: "bg-[#1a1f2e] border-[#1e2028]",
    inputSell: "bg-[#1a1f2e] border-[#1e2028]",
    tabActive: "bg-[#3b82f6] text-white",
    tabInactive: "bg-[#111318] text-slate-300 hover:bg-[#1e2330] border border-[#1e2028]",
    accent: "text-[#3b82f6]",
    accentBg: "bg-[#3b82f6] hover:bg-[#2563eb]",
    subtext: "text-slate-400",
    text: "text-white",
    textSecondary: "text-slate-300",
    textHover: "hover:text-white",
    soldCard: "bg-[#1a1f2e] border-[#1e2028]",
    sidebarItem: "bg-[#1e2330] hover:bg-[#252b3a] border-[#1e2028]",
    header: "bg-gradient-to-b from-[#181c24] to-[#0d1117] border border-[#1e2028]",
    chartGrid: "#1e2028",
    chartAxis: SAP_CHART_COLORS.neutral,
    chartTooltipBg: "#111318",
    chartTooltipBorder: "#3b82f650",
    chartLine: "#3b82f6",
    chartColors: ["#3b82f6", "#22c55e", "#f59e0b", "#ef4444", "#8b5cf6", "#06b6d4"],
  },

  // ── 3. Light ─────────────────────────────────────────────────────────────
  light: {
    name: "Light",
    dot: "bg-blue-500",
    dotColor: "#f0f2f5",
    bg: "bg-[#e8eaed]",
    card: "bg-[#f0f2f5]",
    cardHover: "hover:border-slate-400",
    cardBorder: "border-slate-300",
    panel: "bg-[#f0f2f5]",
    panelBorder: "border-slate-300",
    input: "bg-[#e2e5e9] border-slate-300 text-slate-900 placeholder-slate-400",
    inputSell: "bg-[#e2e5e9] border-slate-300 text-slate-900 placeholder-slate-400",
    tabActive: "bg-blue-600 text-white",
    tabInactive: "bg-[#f0f2f5] text-slate-700 hover:bg-slate-200 border border-slate-300",
    accent: "text-blue-600",
    accentBg: "bg-blue-600 hover:bg-blue-500",
    subtext: "text-slate-600",
    text: "text-slate-900",
    textSecondary: "text-slate-700",
    textHover: "hover:text-slate-900",
    itemHoverBg: "hover:bg-black/5",
    soldCard: "bg-[#e2e5e9] border-slate-300",
    sidebarItem: "bg-[#e2e5e9] hover:bg-slate-300 border-slate-300",
    header: "bg-[#f0f2f5] border-b border-slate-300",
    chartGrid: "#cbd5e1",
    chartAxis: "#64748b",
    chartTooltipBg: "#f0f2f5",
    chartTooltipBorder: "#cbd5e1",
    chartLine: SAP_CHART_COLORS[1],
    chartColors: [SAP_CHART_COLORS[1], SAP_CHART_COLORS[6], SAP_CHART_COLORS[3], SAP_CHART_COLORS[5], SAP_CHART_COLORS[2], SAP_CHART_COLORS[4]],
  },

  // ── 4. Arctic ───────────────────────────────────────────────────────────
  arctic: {
    name: "Arctic",
    dot: "bg-sky-400",
    dotColor: "#38bdf8",
    bg: "bg-[#1a2332]",
    card: "bg-[#1f2d40]",
    cardHover: "hover:border-sky-600/40",
    cardBorder: "border-slate-500/30",
    panel: "bg-[#1c2738]",
    panelBorder: "border-slate-500/30",
    input: "bg-[#243447] border-slate-500/40",
    inputSell: "bg-[#243447] border-slate-500/40",
    tabActive: "bg-sky-500/80 text-white",
    tabInactive: "bg-[#1f2d40] text-slate-300 hover:bg-slate-600/30 border border-slate-500/30",
    accent: "text-sky-400",
    accentBg: "bg-sky-500/80 hover:bg-sky-500",
    subtext: "text-slate-400",
    text: "text-white",
    textSecondary: "text-slate-300",
    textHover: "hover:text-white",
    soldCard: "bg-[#243447] border-slate-500/25",
    sidebarItem: "bg-slate-600/30 hover:bg-slate-600/50 border-slate-500/30",
    header: "bg-gradient-to-b from-[#263750] to-[#1c2738] border border-slate-500/20",
    chartGrid: "#2d3f55",
    chartAxis: SAP_CHART_COLORS.neutral,
    chartTooltipBg: "#1f2d40",
    chartTooltipBorder: "#3d5570",
    chartLine: SAP_CHART_COLORS[6],
    chartColors: [SAP_CHART_COLORS[6], SAP_CHART_COLORS[1], SAP_CHART_COLORS[3], SAP_CHART_COLORS[5], SAP_CHART_COLORS[9], SAP_CHART_COLORS[12]],
  },

  // ── 3. Teal (SAP #6 – #049f9a) ──────────────────────────────────────────
  teal: {
    name: "Deep Teal",
    dot: "bg-[#049f9a]",
    dotColor: "#049f9a",
    bg: "bg-[#081614]",
    card: "bg-[#0d2120]",
    cardHover: "hover:border-teal-600/40",
    cardBorder: "border-teal-900/50",
    panel: "bg-[#0a1c1b]",
    panelBorder: "border-teal-900/50",
    input: "bg-[#102625] border-teal-800/40",
    inputSell: "bg-[#102625] border-teal-800/40",
    tabActive: "bg-[#049f9a] text-white",
    tabInactive: "bg-[#0d2120] text-slate-300 hover:bg-teal-900/30 border border-teal-900/50",
    accent: "text-[#049f9a]",
    accentBg: "bg-[#049f9a] hover:bg-[#06b8b2]",
    subtext: "text-teal-300/60",
    text: "text-white",
    textSecondary: "text-slate-300",
    textHover: "hover:text-white",
    soldCard: "bg-[#102625] border-teal-900/40",
    sidebarItem: "bg-teal-900/30 hover:bg-teal-900/50 border-teal-800/30",
    header: "bg-gradient-to-b from-[#122a28] to-[#0a1c1b] border border-teal-900/40",
    chartGrid: "#1a3533",
    chartAxis: SAP_CHART_COLORS.neutral,
    chartTooltipBg: "#0d2120",
    chartTooltipBorder: "#049f9a50",
    chartLine: SAP_CHART_COLORS[6],
    chartColors: [SAP_CHART_COLORS[6], SAP_CHART_COLORS[1], SAP_CHART_COLORS[3], SAP_CHART_COLORS[9], SAP_CHART_COLORS[12], SAP_CHART_COLORS[5]],
  },

  // ── 4. Indigo (SAP #11 – #5d36ff) ──────────────────────────────────────
  indigo: {
    name: "Indigo",
    dot: "bg-[#5d36ff]",
    dotColor: "#5d36ff",
    bg: "bg-[#0d0b1e]",
    card: "bg-[#14113a]",
    cardHover: "hover:border-indigo-500/40",
    cardBorder: "border-indigo-900/50",
    panel: "bg-[#100e2e]",
    panelBorder: "border-indigo-900/50",
    input: "bg-[#1a1748] border-indigo-800/40",
    inputSell: "bg-[#1a1748] border-indigo-800/40",
    tabActive: "bg-[#5d36ff] text-white",
    tabInactive: "bg-[#14113a] text-slate-300 hover:bg-indigo-900/30 border border-indigo-900/50",
    accent: "text-[#8b6fff]",
    accentBg: "bg-[#5d36ff] hover:bg-[#7050ff]",
    subtext: "text-indigo-300/60",
    text: "text-white",
    textSecondary: "text-slate-300",
    textHover: "hover:text-white",
    soldCard: "bg-[#1a1748] border-indigo-900/40",
    sidebarItem: "bg-indigo-900/30 hover:bg-indigo-900/50 border-indigo-800/30",
    header: "bg-gradient-to-b from-[#1e1a48] to-[#100e2e] border border-indigo-900/40",
    chartGrid: "#1e1a40",
    chartAxis: SAP_CHART_COLORS.neutral,
    chartTooltipBg: "#14113a",
    chartTooltipBorder: "#5d36ff50",
    chartLine: SAP_CHART_COLORS[11],
    chartColors: [SAP_CHART_COLORS[11], SAP_CHART_COLORS[5], SAP_CHART_COLORS[8], SAP_CHART_COLORS[1], SAP_CHART_COLORS[4], SAP_CHART_COLORS[6]],
  },

  // ── 5. Soft Lavender (coolors palette: 22223B / 4A4E69 / 9A8C98 / C9ADA7 / F2E9E4) ──
  lavender: {
    name: "Soft Lavender",
    dot: "bg-[#C9ADA7]",
    dotColor: "#C9ADA7",
    bg: "bg-[#22223B]",
    card: "bg-[#2c2c4a]",
    cardHover: "hover:border-[#9A8C98]/50",
    cardBorder: "border-[#4A4E69]/50",
    panel: "bg-[#262642]",
    panelBorder: "border-[#4A4E69]/50",
    input: "bg-[#2c2c4a] border-[#4A4E69]/50",
    inputSell: "bg-[#2c2c4a] border-[#4A4E69]/50",
    tabActive: "bg-[#9A8C98] text-white",
    tabInactive: "bg-[#2c2c4a] text-[#F2E9E4]/70 hover:bg-[#4A4E69]/40 border border-[#4A4E69]/50",
    accent: "text-[#C9ADA7]",
    accentBg: "bg-[#9A8C98] hover:bg-[#aa9ca8]",
    subtext: "text-[#C9ADA7]/70",
    text: "text-white",
    textSecondary: "text-slate-300",
    textHover: "hover:text-white",
    soldCard: "bg-[#2c2c4a] border-[#4A4E69]/40",
    sidebarItem: "bg-[#4A4E69]/30 hover:bg-[#4A4E69]/50 border-[#4A4E69]/40",
    header: "bg-gradient-to-b from-[#363655] to-[#262642] border border-[#4A4E69]/40",
    chartGrid: "#3a3b5e",
    chartAxis: "#9A8C98",
    chartTooltipBg: "#2c2c4a",
    chartTooltipBorder: "#9A8C9870",
    chartLine: "#C9ADA7",
    chartColors: ["#C9ADA7", "#9A8C98", "#F2E9E4", "#4A6e9e", "#a07a8a", "#7a8c9a"],
  },

  // ── 6. SAP Fiori (SAP corporate dark, royal blue accent) ────────────────
  sap: {
    name: "SAP Fiori",
    dot: "bg-[#0070f2]",
    dotColor: "#0070f2",
    bg: "bg-[#0a1626]",
    card: "bg-[#172638]",
    cardHover: "hover:border-[#0070f2]/40",
    cardBorder: "border-slate-600/30",
    panel: "bg-[#13202f]",
    panelBorder: "border-slate-600/30",
    input: "bg-[#1a2b40] border-slate-600/40",
    inputSell: "bg-[#1a2b40] border-slate-600/40",
    tabActive: "bg-[#0070f2] text-white",
    tabInactive: "bg-[#172638] text-slate-300 hover:bg-slate-700/30 border border-slate-600/30",
    accent: "text-[#168eff]",
    accentBg: "bg-[#0070f2] hover:bg-[#168eff]",
    subtext: "text-slate-400",
    text: "text-white",
    textSecondary: "text-slate-300",
    textHover: "hover:text-white",
    soldCard: "bg-[#1a2b40] border-slate-600/25",
    sidebarItem: "bg-slate-700/30 hover:bg-slate-700/50 border-slate-600/40",
    header: "bg-gradient-to-b from-[#1c3045] to-[#13202f] border border-slate-600/20",
    chartGrid: "#243a55",
    chartAxis: SAP_CHART_COLORS.neutral,
    chartTooltipBg: "#172638",
    chartTooltipBorder: "#0070f250",
    chartLine: SAP_CHART_COLORS[7],
    chartColors: [SAP_CHART_COLORS[7], SAP_CHART_COLORS[1], SAP_CHART_COLORS[6], SAP_CHART_COLORS[2], SAP_CHART_COLORS[4], SAP_CHART_COLORS[5]],
  },

  // ── 7. Graphite (minimal monochrome, low contrast) ──────────────────────
  graphite: {
    name: "Graphite",
    dot: "bg-zinc-300",
    dotColor: "#d4d4d8",
    bg: "bg-[#0e0e10]",
    card: "bg-[#18181b]",
    cardHover: "hover:border-zinc-500/40",
    cardBorder: "border-zinc-700/40",
    panel: "bg-[#141416]",
    panelBorder: "border-zinc-700/40",
    input: "bg-[#1c1c1f] border-zinc-700/40",
    inputSell: "bg-[#1c1c1f] border-zinc-700/40",
    tabActive: "bg-zinc-200 text-zinc-900",
    tabInactive: "bg-[#18181b] text-zinc-300 hover:bg-zinc-800/50 border border-zinc-700/40",
    accent: "text-zinc-300",
    accentBg: "bg-zinc-700 hover:bg-zinc-600",
    subtext: "text-zinc-500",
    text: "text-white",
    textSecondary: "text-slate-300",
    textHover: "hover:text-white",
    soldCard: "bg-[#1c1c1f] border-zinc-700/30",
    sidebarItem: "bg-zinc-800/40 hover:bg-zinc-800/60 border-zinc-700/40",
    header: "bg-gradient-to-b from-[#222226] to-[#141416] border border-zinc-700/30",
    chartGrid: "#2a2a2d",
    chartAxis: "#71717a",
    chartTooltipBg: "#18181b",
    chartTooltipBorder: "#3f3f46",
    chartLine: "#a1a1aa",
    chartColors: ["#a1a1aa", "#71717a", "#d4d4d8", "#52525b", "#27272a", "#f4f4f5"],
  },

  // ── 9. Olive (SAP #3 – #75980b) ─────────────────────────────────────────
  olive: {
    name: "Olive",
    dot: "bg-[#75980b]",
    dotColor: "#75980b",
    bg: "bg-[#0e1209]",
    card: "bg-[#161d0d]",
    cardHover: "hover:border-lime-700/40",
    cardBorder: "border-lime-900/40",
    panel: "bg-[#12170a]",
    panelBorder: "border-lime-900/40",
    input: "bg-[#1c2510] border-lime-800/40",
    inputSell: "bg-[#1c2510] border-lime-800/40",
    tabActive: "bg-[#75980b] text-white",
    tabInactive: "bg-[#161d0d] text-slate-300 hover:bg-lime-900/30 border border-lime-900/40",
    accent: "text-[#9ec41a]",
    accentBg: "bg-[#75980b] hover:bg-[#87ae0d]",
    subtext: "text-lime-300/50",
    text: "text-white",
    textSecondary: "text-slate-300",
    textHover: "hover:text-white",
    soldCard: "bg-[#1c2510] border-lime-900/30",
    sidebarItem: "bg-lime-900/20 hover:bg-lime-900/40 border-lime-800/30",
    header: "bg-gradient-to-b from-[#1e2a10] to-[#12170a] border border-lime-900/30",
    chartGrid: "#1e2a10",
    chartAxis: SAP_CHART_COLORS.neutral,
    chartTooltipBg: "#161d0d",
    chartTooltipBorder: "#75980b50",
    chartLine: SAP_CHART_COLORS[3],
    chartColors: [SAP_CHART_COLORS[3], SAP_CHART_COLORS[6], SAP_CHART_COLORS[9], SAP_CHART_COLORS[12], SAP_CHART_COLORS[1], SAP_CHART_COLORS[2]],
  },
};